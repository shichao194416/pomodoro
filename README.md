...
**其他**
- **横屏专注模式**：手机横过来自动进入全屏沉浸界面 —— 只有发光进度环、大倒计时、开始/暂停按钮和右上角「退出横屏」。其余界面（顶栏、底栏、统计）**整体卸载**，DOM 里都干净
- 退出专注模式后回到普通界面（横屏时倒计时居中），顶栏的 ⤢ 按钮可以随时回到专注模式
- 顶栏是自定义鼓励语，改 `src/config.ts` 里的 `HEADER_GREETING` 即可
- 离线可用：Service Worker 预缓存全部资源（含提示音）
- 深色模式，6 种语言（简体中文 / English / Español / Français / Esperanto / Русский）

> **已移除的界面**：标签选择器与备注输入框（用户要求精简）。相关代码 `TagSelector.tsx`、`SessionNote.tsx` 仍在仓库里但不再挂载。
> 注意「总结」页还留着按标签筛选的分支——没有标签选择器之后它只剩一个 `General`，需要的话可以一并去掉。
